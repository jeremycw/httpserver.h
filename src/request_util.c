...
http_string_t hs_request_header(http_request_t* request, char const * key) {
  ...
  for (int i = 0; i < request->tokens.size; i++) {
    ...
    if (token.type == HSH_TOK_HEADER_KEY && token.len == len) {
      if (_hs_case_insensitive_cmp(&request->buffer.buf[token.index], key, len)) {
...
void hs_auto_detect_keep_alive(http_request_t* request) {
  ...
  if (
    (str.len == 5 && _hs_case_insensitive_cmp(str.buf, "close", 5)) ||
...
int _hs_assign_iteration_headers(
  ...
  if (request->tokens.buf[*iter].type == HSH_TOK_BODY) return 0;
  *key = (http_string_t) {
    .buf = &request->buffer.buf[token.index],
    .len = token.len
  };
