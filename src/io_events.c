...
void hs_connection_io_cb(struct kevent* ev) {
  ...
  if (ev->filter == EVFILT_TIMER) {
    request->timeout -= 1;
    if (request->timeout == 0) hs_terminate_connection(request);
  } else {
    _hs_connection_process_io(request);
  }
}
...
void hs_accept_cb(struct kevent* ev) {
  ...
  if (ev->filter == EVFILT_TIMER) {
    ...
    hs_accept_connections(server, hs_connection_io_cb, _hs_mem_error_responder, HTTP_MAX_TOTAL_EST_MEM_USAGE);
  }
}
...
void hs_accept_cb(struct epoll_event* ev) {
  hs_accept_connections((http_server_t*)ev->data.ptr, hs_connection_io_cb, _hs_mem_error_responder);
}
